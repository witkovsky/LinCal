...
if isempty(uX), uX0 = 0; end
if isempty(uY), uY0 = 0; end
...
for i = 1:(order+1)
    wX = LX(i,:);
    wY = KY(i,:);
    cfPars{i} = WeightedCF(wX,wY,cfX,cfX0,cfY,cfY0);
end
...
if ~isempty(yLow) && ~isempty(yUpp)
    [xLow,xUpp] = CalInt(yLow,yUpp,xMin,xMax,probLow,probUpp,order,...
        LX,KY,cfX,cfX0,cfY,cfY0,parsEstimate,options);
else
    xLow = [];
    xUpp = [];
end
